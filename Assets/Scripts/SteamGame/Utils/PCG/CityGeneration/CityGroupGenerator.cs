...
{
    public class CityGroupGenerator : Singleton<CityGroupGenerator>
    {
        [Header("Customisable Attribute")] [SerializeField]
        private int seed;

        [Range(1, 5)] [SerializeField] private int column; // x
        [Range(1, 5)] [SerializeField] private int row; // y

        [Space(10)] [Range(3, 10)] [SerializeField]
        private int lotColumn; // x

        [Range(3, 10)] [SerializeField] private int lotRow; // y

        [Space(10)] [Range(0, 10)] [SerializeField]
        private int maximumLotRoadCount;

        [Space(20), Header("Others Attribute")] [SerializeField]
        private GameObject cityBase;

        [Space(20), Header("Sub City Attribute")] [SerializeField]
        private Transform subCityParent;
        ...
        [Space(20), Header("Mark Attribute")] [SerializeField]
        private Transform markParent;

        [SerializeField] private CityMark markPrefab;

        [Space(20), Header("Edge Building Attribute")] [SerializeField]
        ...
        private void GenerateEdgeBuilding()
        {
            ...
            void ConstructEdgeBuilding(int x, int y, StepMoveDirectionType facingSide)
            {
                ...
                newBuildingMark.transform.position =
                    new Vector3(x * CityUtility.MARKS_SPACE, 0f, y * CityUtility.MARKS_SPACE);
        ...
        private void GenerateMainRoad()
        {
            ...
            void PlantMainRoadMark(int x, int y)
            {
                ...
                newMainRoadMark.transform.position =
                    new Vector3(x * CityUtility.MARKS_SPACE, 0f, y * CityUtility.MARKS_SPACE);
        ...
        private void ViasualizeMainRoad()
        {
            ...
            Vector3 ComputeRoundRoadPosition(int x, int y) =>
                new Vector3(x * CityUtility.MARKS_SPACE, 0f, y * CityUtility.MARKS_SPACE);
        }
        ...
        {
            var xScale = (column * lotColumn * CityUtility.MARKS_SPACE) + (2 * CityUtility.MARKS_SPACE) -
                         ((column - 1) * CityUtility.MARKS_SPACE);
            var yScale = (row * lotRow * CityUtility.MARKS_SPACE) + (2 * CityUtility.MARKS_SPACE) -
                         ((row - 1) * CityUtility.MARKS_SPACE);
