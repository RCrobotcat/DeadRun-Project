...
public class MaterialToLayerRenderPass : ScriptableRenderPass
{
    ...
    public MaterialToLayerRenderPass(LayerMask layerMask, Material material, RenderQueueRange renderQueueRange)
    {
        ...
        var forwardOnlyShaderTagIds = new[]
        {
            ...
            new ShaderTagId(
                "SRPDefaultUnlit"), // Legacy shaders (do not have a gbuffer pass) are considered forward-only for backward compatibility
            new ShaderTagId(
                "LightweightForward") // Legacy shaders (do not have a gbuffer pass) are considered forward-only for backward compatibility
        };
    ...
    private void InitRendererLists(ContextContainer frameData, ref LayerRenderPassData layerRenderPassData,
        RenderGraph renderGraph)
    {
        ...
        var drawSettings = RenderingUtils.CreateDrawingSettings(_shaderTagIdList, universalRenderingData, cameraData,
    ...
    public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
    {
        ...
        using (var builder =
               renderGraph.AddRasterRenderPass<LayerRenderPassData>(layerRenderPassName, out var passData))
        {
            ...
            builder.SetRenderFunc((LayerRenderPassData data, RasterGraphContext context) =>
                ExecuteLayerRenderPass(data, context));
        }
